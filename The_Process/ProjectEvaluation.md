...
Looking at the project from a maintainable point of view, we believe by using the MEAN stack, and in particular Angular for the front-end, we have made it easy to expand upon and improve further down the line. The use of components help keep the code base DRY for certain aspects of the website, such as navgiation toolbars and social footers. Further, with a purely JavaScript/TypeScript code base, the language is consistent from front-end to back-end so as a result, adding new quizzes and features would be straight forward.
